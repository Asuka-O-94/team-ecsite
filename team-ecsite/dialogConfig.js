		...
		buttons: [
			{
				...
				click: function() {
					...
					$.ajax({
						type: 'POST',
						url: '/chocolate/auth/resetPassword',
						data: JSON.stringify(jsonString),
						contentType: 'application/json',
						scriptCharset: 'utf-8'
					})
		...
		buttons: [
			{
				...
				click: function() {
					...
					$.ajax({
						type: 'POST',
						url: '/chocolate/destination/register',
						data: JSON.stringify(jsonString),
						contentType: 'application/json',
						datatype: 'json',
						scriptCharset: 'utf-8'
...
function settlement(destinationId) {
	$.ajax({
		type: 'POST',
		url: '/chocolate/settlement/complete',
		data: JSON.stringify({'destinationId': destinationId}),
		datatype: 'json',
		contentType: 'application/json',
	})
	.then((result) => {
		location.replace('/chocolate/history/');
	}, () => {
		alert('Error: ajax connection failed.');
	});
}
